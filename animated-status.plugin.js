//META{"name":"AnimatedStatus","source":"https://github.com/Skyriiis/StatutAnim-/blob/main/animated-status.plugin.js","website":"https://github.com/Skyriiis/StatutAnim-"}*//
...
class AnimatedStatus {
	...
	getDescription() {
		return "Plugins pour animé ton statut discord";
	}
	...
	Status_Animate(index = 0) {
		...
		if (this.animation[index] == undefined) {
			BdApi.showToast("Animated Status: Pas de statut ? Va le configurer dans Plugins/setting");
			return;
		}
	...
	newRichRow(text, emoji, optNitroId = undefined) {
		...
		textWidget.placeholder = "Zone de texte";
		...
		emojiWidget.placeholder = "Nom emote";
	...
	getSettingsPanel() {
		...
		// timeout
		settings.appendChild(GUI.newLabel("Temps changement (minimum 2900 milisecondes)"));
		...
		let changeEditMode = GUI.newButton("Changer le mode d'écriture");
		...
		let save = GUI.newButton("Sauvegarder");
		...
		save.onclick = () => {
			...
			// Show Toast
			BdApi.showToast("Les settings sont save!", {type: "success"});
